...
class ChannelList extends React.Component {
  ...
  render() {
    ...
    const isOwner = this.props.servers[this.props.match.params.serverId].ownerId === this.props.currentUserId;
    const channelItems =
      channels.length > 0 ? 
        channels.map(channel => <ChannelListItemContainer key={channel.id} channel={channel} changeActiveChannel={this.props.changeActiveChannel} isOwner={isOwner}/>)
        : null;
